...
import { Edit, Smartphone, Save, X, ArrowLeft } from "lucide-react";
...
const UpdateStock = () => {
  ...
  const { updateMobile, getMobileById } = useInventory();
  const { toast } = useToast();

  const [formData, setFormData] = useState<Partial<MobilePhone>>({});
  ...
  useEffect(() => {
    if (id) {
      const mobile = getMobileById(id);
      if (mobile) {
        setFormData(mobile);
      } else {
        toast({
          title: "Error",
          description: "Mobile phone not found",
          variant: "destructive",
        });
        navigate("/inventory");
      }
    ...
    setIsLoading(false);
  }, [id, getMobileById, navigate, toast]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      updateMobile(id, formData);
      
      toast({
        title: "Stock Updated Successfully",
        description: `${formData.brand} ${formData.modelName} has been updated`,
        variant: "default",
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground ml-12">
            Modify details for {formData.brand} {formData.modelName}
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => handleInputChange("osPlatform", value)}
                  >
                    <SelectTrigger>
